...
export const impressoraOuAdminMiddlewareOuManager = (req: Request, res: Response, next: NextFunction) => {
  if (req.user?.role !== 'admin' && req.user?.role !== 'tecnico_impressora' && req.user?.role !== 'gerente') {
    return res.status(403).json({ message: 'Acesso negado. Requer perfil de Administrador, Gerente ou Técnico de Impressora.' });
  }
  next();
};

export const eletronicauAdminMiddlewareOuManager = (req: Request, res: Response, next: NextFunction) => {
  if (req.user?.role !== 'admin' && req.user?.role !== 'tecnico_eletronica' && req.user?.role !== 'gerente') {
    return res.status(403).json({ message: 'Acesso negado. Requer perfil de Administrador, Gerente ou Técnico de Eletronica.' });
  }
  next();
};